...
const SOURCE_PATH = path.join(__dirname, '..', 'data', 'escort-terms.txt');

const PAGE_SLUG = 'escort-terms';
const PAGE_TITLE = '陪诊员服务协议';
const PAGE_EXCERPT = '科科灵陪诊平台陪诊师端用户服务协议';
...
function buildEscortTermsHtml(rawText: string) {
...
async function main() {
  ...
  const content = buildEscortTermsHtml(rawText);
  const now = new Date();
  ...
  console.log(`[escort-terms] upsert success: ${page.slug}`);
}

main()
  .catch((error) => {
    console.error('[escort-terms] upsert failed:', error);
    process.exit(1);
  })
  .finally(async () => {
