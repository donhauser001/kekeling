...
> **关联文档**: [03-任务卡拆解.md](./03-任务卡拆解.md) · [04-P2审核打款设计.md](./04-P2审核打款设计.md) · [06-组件Props契约.md](./06-组件Props契约.md)  
> **代码契约**: `src/features/withdrawals/contracts.ts`
...
- [ ] `pending` 状态是否只出现"审核"
- [ ] `approved` 状态是否只出现"打款"
- [ ] `processing` / `completed` / `failed` 是否无操作按钮

### 高危操作

- [ ] 打款是否使用 Modal（非 Drawer）
- [ ] 打款 Modal 是否包含关键信息复核
- [ ] 打款 Modal 是否需要输入 CONFIRM
- [ ] CONFIRM 输入框是否禁止粘贴

### 错误处理

- [ ] 409 冲突是否有明确提示并引导刷新
- [ ] 401/403/5xx 是否有统一错误提示
- [ ] 网络错误是否有兜底提示
