# 科科灵 - 陪诊员模块开发规划文档

**版本**: v1.1  
**创建日期**: 2024-12-08  
**更新日期**: 2024-12-08  
**状态**: ✅ Schema 已更新，待开发 CRUD

---

## 更新记录

| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v1.1 | 2024-12-08 | 根据审计修复：添加 `userId`, `cityCode`, `workStatus` 字段；添加种子数据 |
| v1.0 | 2024-12-08 | 初稿 |

---

## 一、模块概述

### 1.1 功能定位

陪诊员是平台的核心服务提供者，负责执行用户预订的陪诊服务。本模块需要支持：

- 陪诊员信息的完整生命周期管理
- 陪诊员与医院的多对多关联
- 陪诊员资质审核与状态管理
- 为订单派单提供可用陪诊员池

### 1.2 现状分析

| 部分 | 完成度 | 说明 |
|------|--------|------|
| 数据库模型 | ✅ 100% | `Escort` + `EscortHospital` 已定义 |
| 后端查询 API | ✅ 60% | 列表/详情/推荐已实现 |
| 后端 CRUD API | ❌ 0% | 创建/编辑/删除未实现 |
| 关联医院 API | ❌ 0% | 未实现 |
| 前端表格展示 | ✅ 80% | 基础展示已完成 |
| 前端 CRUD 功能 | ❌ 0% | 对话框未对接真实 API |
| 种子数据 | ❌ 0% | 无测试数据 |

---

## 二、数据模型

### 2.1 Prisma Schema (已更新)

```prisma
model Escort {
  id             String    @id @default(uuid())
  userId         String?   @unique @map("user_id") // ✅ 新增：关联小程序账号
  name           String
  gender         String    // male, female
  phone          String    @unique
  idCard         String?   @map("id_card")
  avatar         String?

  // 业务属性
  cityCode       String    @default("110100") @map("city_code") // ✅ 新增：城市代码
  level          String    // senior, intermediate, junior, trainee
  experience     String?   // 从业年限
  introduction   String?   // 个人简介
  tags           String?   // JSON: ["耐心细致", "经验丰富"]
  certificates   String?   // JSON: [{name, url, expireDate}]

  // 统计数据
  rating         Float     @default(5.0)  // 评分 (5分制)
  orderCount     Int       @default(0) @map("order_count")

  // 状态管理
  status         String    @default("pending") // pending, active, inactive, suspended
  workStatus     String    @default("resting") @map("work_status") // ✅ 新增：resting, working, busy

  createdAt      DateTime  @default(now()) @map("created_at")
  updatedAt      DateTime  @updatedAt @map("updated_at")

  // 关联
  user           User?     @relation(fields: [userId], references: [id])
  hospitals      EscortHospital[]
  orders         Order[]

  @@map("escorts")
}

model EscortHospital {
  id           String   @id @default(uuid())
  escortId     String   @map("escort_id")
  hospitalId   String   @map("hospital_id")
  familiarDepts String? @map("familiar_depts") // JSON: ["内科", "外科"]

  escort       Escort   @relation(fields: [escortId], references: [id])
  hospital     Hospital @relation(fields: [hospitalId], references: [id])

  @@unique([escortId, hospitalId])
  @@map("escort_hospitals")
}
```

### 2.2 新增字段说明

| 字段 | 类型 | 说明 | 业务用途 |
|------|------|------|----------|
| `userId` | `String?` | 关联 User 表 | 小程序登录后自动绑定 |
| `cityCode` | `String` | 城市代码 | 派单时按城市筛选 |
| `workStatus` | `String` | 接单状态 | 派单时排除休息/忙碌的陪诊员 |

### 2.3 账号关联逻辑

```
Admin 创建陪诊员 (phone: 13800138001)
         ↓
用户在小程序登录并绑定手机号 13800138001
         ↓
后端自动匹配: UPDATE escorts SET user_id = ? WHERE phone = ?
         ↓
该用户进入小程序时识别为陪诊员，显示陪诊员专属功能
```

---

## 三、后端 API 设计

### 3.1 API 端点规划

#### 管理端 API (`/api/admin/escorts`)

| 方法 | 端点 | 说明 | 优先级 |
|------|------|------|--------|
| GET | `/admin/escorts` | 获取陪诊员列表（分页、筛选） | ✅ 已实现 |
| GET | `/admin/escorts/available` | 获取可派单陪诊员 | ✅ 已实现 |
| GET | `/admin/escorts/:id` | 获取陪诊员详情 | 🔴 待实现 |
| POST | `/admin/escorts` | 创建陪诊员 | 🔴 待实现 |
| PUT | `/admin/escorts/:id` | 更新陪诊员信息 | 🔴 待实现 |
| DELETE | `/admin/escorts/:id` | 删除陪诊员 | 🔴 待实现 |
| PUT | `/admin/escorts/:id/status` | 更新陪诊员状态 | 🔴 待实现 |
| POST | `/admin/escorts/:id/hospitals` | 关联医院 | 🔴 待实现 |
| DELETE | `/admin/escorts/:id/hospitals/:hospitalId` | 解除医院关联 | 🔴 待实现 |

#### 用户端 API (`/api/escorts`)

| 方法 | 端点 | 说明 | 状态 |
|------|------|------|------|
| GET | `/escorts` | 获取陪诊员列表 | ✅ 已实现 |
| GET | `/escorts/recommended` | 获取推荐陪诊员 | ✅ 已实现 |
| GET | `/escorts/:id` | 获取陪诊员详情 | ✅ 已实现 |

### 3.2 DTO 定义

```typescript
// server/src/modules/escorts/dto/escort.dto.ts

export class CreateEscortDto {
  name: string;
  gender: 'male' | 'female';
  phone: string;
  avatar?: string;
  idCard?: string;
  level: 'senior' | 'intermediate' | 'junior' | 'trainee';
  experience?: string;
  introduction?: string;
  tags?: string[];
  certificates?: Array<{
    name: string;
    url: string;
    expireDate?: string;
  }>;
  hospitalIds?: string[]; // 初始关联的医院
}

export class UpdateEscortDto {
  name?: string;
  gender?: 'male' | 'female';
  phone?: string;
  avatar?: string;
  idCard?: string;
  level?: 'senior' | 'intermediate' | 'junior' | 'trainee';
  experience?: string;
  introduction?: string;
  tags?: string[];
  certificates?: Array<{
    name: string;
    url: string;
    expireDate?: string;
  }>;
  status?: 'active' | 'inactive' | 'pending' | 'suspended';
}

export class AssociateHospitalDto {
  hospitalId: string;
  familiarDepts?: string[]; // 熟悉的科室
}
```

### 3.3 Service 实现要点

```typescript
// server/src/modules/admin/services/admin-escorts.service.ts

@Injectable()
export class AdminEscortsService {
  constructor(private prisma: PrismaService) {}

  // 创建陪诊员
  async create(dto: CreateEscortDto) {
    const { hospitalIds, tags, certificates, ...data } = dto;
    
    return this.prisma.escort.create({
      data: {
        ...data,
        tags: tags ? JSON.stringify(tags) : null,
        certificates: certificates ? JSON.stringify(certificates) : null,
        // 同时创建医院关联
        hospitals: hospitalIds?.length ? {
          create: hospitalIds.map(hospitalId => ({ hospitalId }))
        } : undefined,
      },
      include: {
        hospitals: { include: { hospital: true } }
      }
    });
  }

  // 更新陪诊员
  async update(id: string, dto: UpdateEscortDto) {
    const { tags, certificates, ...data } = dto;
    
    return this.prisma.escort.update({
      where: { id },
      data: {
        ...data,
        tags: tags !== undefined ? JSON.stringify(tags) : undefined,
        certificates: certificates !== undefined ? JSON.stringify(certificates) : undefined,
      },
      include: {
        hospitals: { include: { hospital: true } }
      }
    });
  }

  // 删除陪诊员
  async delete(id: string) {
    // 检查是否有进行中的订单
    const activeOrders = await this.prisma.order.count({
      where: {
        escortId: id,
        status: { in: ['pending', 'accepted', 'in_progress'] }
      }
    });
    
    if (activeOrders > 0) {
      throw new BadRequestException('该陪诊员还有进行中的订单，无法删除');
    }
    
    // 级联删除医院关联
    await this.prisma.escortHospital.deleteMany({ where: { escortId: id } });
    return this.prisma.escort.delete({ where: { id } });
  }

  // 关联医院
  async associateHospital(escortId: string, dto: AssociateHospitalDto) {
    return this.prisma.escortHospital.upsert({
      where: {
        escortId_hospitalId: {
          escortId,
          hospitalId: dto.hospitalId,
        }
      },
      create: {
        escortId,
        hospitalId: dto.hospitalId,
        familiarDepts: dto.familiarDepts ? JSON.stringify(dto.familiarDepts) : null,
      },
      update: {
        familiarDepts: dto.familiarDepts ? JSON.stringify(dto.familiarDepts) : null,
      },
    });
  }

  // 解除医院关联
  async dissociateHospital(escortId: string, hospitalId: string) {
    return this.prisma.escortHospital.delete({
      where: {
        escortId_hospitalId: { escortId, hospitalId }
      }
    });
  }
}
```

---

## 四、前端实现方案

### 4.1 API 层扩展

```typescript
// src/lib/api.ts

export interface Escort {
  id: string
  name: string
  gender: 'male' | 'female'
  avatar: string | null
  phone: string
  idCard: string | null
  level: 'senior' | 'intermediate' | 'junior' | 'trainee'
  experience: string | null
  introduction: string | null
  tags: string[] | null        // 前端解析后的数组
  certificates: Certificate[] | null
  rating: number
  orderCount: number
  status: 'active' | 'inactive' | 'pending' | 'suspended'
  hospitals: EscortHospital[]
  createdAt: string
  updatedAt: string
}

export interface EscortHospital {
  id: string
  hospitalId: string
  hospital: { id: string; name: string }
  familiarDepts: string[] | null
}

export interface Certificate {
  name: string
  url: string
  expireDate?: string
}

export interface CreateEscortData {
  name: string
  gender: 'male' | 'female'
  phone: string
  avatar?: string
  idCard?: string
  level: 'senior' | 'intermediate' | 'junior' | 'trainee'
  experience?: string
  introduction?: string
  tags?: string[]
  certificates?: Certificate[]
  hospitalIds?: string[]
}

export const escortApi = {
  // ... 现有方法 ...

  // 创建陪诊员
  create: (data: CreateEscortData) =>
    request<Escort>('/admin/escorts', {
      method: 'POST',
      body: JSON.stringify(data),
    }),

  // 更新陪诊员
  update: (id: string, data: Partial<CreateEscortData> & { status?: string }) =>
    request<Escort>(`/admin/escorts/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
    }),

  // 删除陪诊员
  delete: (id: string) =>
    request<void>(`/admin/escorts/${id}`, {
      method: 'DELETE',
    }),

  // 更新状态
  updateStatus: (id: string, status: string) =>
    request<Escort>(`/admin/escorts/${id}/status`, {
      method: 'PUT',
      body: JSON.stringify({ status }),
    }),

  // 关联医院
  associateHospital: (escortId: string, hospitalId: string, familiarDepts?: string[]) =>
    request<void>(`/admin/escorts/${escortId}/hospitals`, {
      method: 'POST',
      body: JSON.stringify({ hospitalId, familiarDepts }),
    }),

  // 解除医院关联
  dissociateHospital: (escortId: string, hospitalId: string) =>
    request<void>(`/admin/escorts/${escortId}/hospitals/${hospitalId}`, {
      method: 'DELETE',
    }),
}
```

### 4.2 表单 Schema (Zod)

```typescript
// src/features/escorts/data/schema.ts

import { z } from 'zod'

export const escortLevels = ['senior', 'intermediate', 'junior', 'trainee'] as const
export const escortStatuses = ['active', 'inactive', 'pending', 'suspended'] as const

export const certificateSchema = z.object({
  name: z.string().min(1, '证书名称不能为空'),
  url: z.string().url('请输入有效的URL'),
  expireDate: z.string().optional(),
})

export const escortFormSchema = z.object({
  name: z.string().min(2, '姓名至少2个字符').max(20),
  gender: z.enum(['male', 'female']),
  phone: z.string().regex(/^1[3-9]\d{9}$/, '请输入有效的手机号'),
  avatar: z.string().optional(),
  idCard: z.string().regex(/^\d{17}[\dXx]$/, '请输入有效的身份证号').optional().or(z.literal('')),
  level: z.enum(escortLevels),
  experience: z.string().optional(),
  introduction: z.string().max(500, '简介最多500字').optional(),
  tags: z.array(z.string()).optional(),
  certificates: z.array(certificateSchema).optional(),
  hospitalIds: z.array(z.string()).optional(),
  status: z.enum(escortStatuses).optional(),
})

export type EscortFormData = z.infer<typeof escortFormSchema>
```

### 4.3 创建/编辑对话框组件

```tsx
// src/features/escorts/components/escort-form-dialog.tsx

export function EscortFormDialog({ 
  open, 
  onOpenChange, 
  escort // 编辑时传入，新建时为 undefined
}: Props) {
  const form = useForm<EscortFormData>({
    resolver: zodResolver(escortFormSchema),
    defaultValues: escort ? {
      name: escort.name,
      gender: escort.gender,
      phone: escort.phone,
      // ... 其他字段
    } : {
      gender: 'male',
      level: 'junior',
      tags: [],
      hospitalIds: [],
    }
  })

  // 标签动态输入
  const [tagInput, setTagInput] = useState('')
  const tags = form.watch('tags') || []

  const addTag = () => {
    if (tagInput && !tags.includes(tagInput)) {
      form.setValue('tags', [...tags, tagInput])
      setTagInput('')
    }
  }

  // 医院选择
  const { data: hospitals } = useHospitals({ pageSize: 100 })
  const selectedHospitalIds = form.watch('hospitalIds') || []

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{escort ? '编辑陪诊员' : '新增陪诊员'}</DialogTitle>
        </DialogHeader>
        
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            {/* 基本信息 */}
            <div className="grid grid-cols-2 gap-4">
              <FormField name="name" ... />
              <FormField name="gender" ... />
              <FormField name="phone" ... />
              <FormField name="level" ... />
            </div>

            {/* 头像上传 */}
            <FormField name="avatar">
              <ImageUpload ... />
            </FormField>

            {/* 个人简介 */}
            <FormField name="introduction">
              <Textarea rows={3} />
            </FormField>

            {/* 服务标签 */}
            <div>
              <Label>服务标签</Label>
              <div className="flex gap-2 flex-wrap mb-2">
                {tags.map(tag => (
                  <Badge key={tag}>
                    {tag}
                    <X onClick={() => removeTag(tag)} />
                  </Badge>
                ))}
              </div>
              <div className="flex gap-2">
                <Input 
                  value={tagInput}
                  onChange={e => setTagInput(e.target.value)}
                  placeholder="输入标签按回车添加"
                  onKeyDown={e => e.key === 'Enter' && addTag()}
                />
                <Button type="button" onClick={addTag}>添加</Button>
              </div>
            </div>

            {/* 关联医院 */}
            <div>
              <Label>关联医院</Label>
              <MultiSelect
                options={hospitals?.data.map(h => ({ value: h.id, label: h.name }))}
                selected={selectedHospitalIds}
                onChange={ids => form.setValue('hospitalIds', ids)}
              />
            </div>

            <DialogFooter>
              <Button type="submit">保存</Button>
            </DialogFooter>
          </form>
        </Form>
      </DialogContent>
    </Dialog>
  )
}
```

---

## 五、种子数据

### 5.1 陪诊员数据 (10 条)

```typescript
// server/prisma/seed.ts

const escortsData = [
  {
    name: '张晓红',
    gender: 'female',
    phone: '13800138001',
    level: 'senior',
    experience: '8年',
    introduction: '从事医疗陪诊服务8年，熟悉北京各大三甲医院就诊流程，服务过上千位患者，深受好评。',
    tags: ['耐心细致', '经验丰富', '三甲医院专家'],
    rating: 99.2,
    orderCount: 1258,
    status: 'active',
    hospitals: ['协和医院', '北大第一医院', '北京医院'],
  },
  {
    name: '李明华',
    gender: 'male',
    phone: '13800138002',
    level: 'senior',
    experience: '6年',
    introduction: '专注肿瘤科陪诊，对北京肿瘤医院、中国医学科学院肿瘤医院非常熟悉。',
    tags: ['肿瘤科专家', '24小时服务', '住院陪护'],
    rating: 98.5,
    orderCount: 876,
    status: 'active',
    hospitals: ['北京肿瘤医院', '中日友好医院'],
  },
  {
    name: '王芳',
    gender: 'female',
    phone: '13800138003',
    level: 'intermediate',
    experience: '4年',
    introduction: '擅长儿科陪诊，有爱心，善于与小朋友沟通，让看病不再可怕。',
    tags: ['儿科专长', '亲和力强', '细心周到'],
    rating: 97.8,
    orderCount: 542,
    status: 'active',
    hospitals: ['北京儿童医院', '首都儿科研究所'],
  },
  {
    name: '陈志强',
    gender: 'male',
    phone: '13800138004',
    level: 'intermediate',
    experience: '3年',
    introduction: '退伍军人，责任心强，擅长老年患者陪护。',
    tags: ['老年护理', '责任心强', '力量型服务'],
    rating: 96.5,
    orderCount: 328,
    status: 'active',
    hospitals: ['北京老年医院', '北京医院', '解放军总医院'],
  },
  {
    name: '刘婷婷',
    gender: 'female',
    phone: '13800138005',
    level: 'junior',
    experience: '2年',
    introduction: '护理专业毕业，持有护士资格证，专业素养高。',
    tags: ['护理专业', '持证上岗', '年轻活力'],
    rating: 95.0,
    orderCount: 186,
    status: 'active',
    hospitals: ['协和医院', '北京大学人民医院'],
  },
  {
    name: '赵伟',
    gender: 'male',
    phone: '13800138006',
    level: 'junior',
    experience: '1年',
    introduction: '认真负责，服务态度好，正在快速成长中。',
    tags: ['态度好', '守时', '学习能力强'],
    rating: 93.2,
    orderCount: 87,
    status: 'active',
    hospitals: ['北京中医药大学东直门医院'],
  },
  {
    name: '孙丽',
    gender: 'female',
    phone: '13800138007',
    level: 'trainee',
    experience: '半年',
    introduction: '实习陪诊员，热情学习中。',
    tags: ['新人', '热情'],
    rating: 90.0,
    orderCount: 25,
    status: 'active',
    hospitals: ['朝阳医院'],
  },
  {
    name: '周强',
    gender: 'male',
    phone: '13800138008',
    level: 'senior',
    experience: '10年',
    introduction: '资深陪诊员，曾任医院护工组长，对医院运作非常了解。',
    tags: ['资深专家', '全科服务', 'VIP专属'],
    rating: 99.5,
    orderCount: 2156,
    status: 'active',
    hospitals: ['协和医院', '北京医院', '解放军301医院', '北大第一医院'],
  },
  {
    name: '吴敏',
    gender: 'female',
    phone: '13800138009',
    level: 'intermediate',
    experience: '5年',
    introduction: '妇产科陪诊专家，陪伴过上百位准妈妈完成产检和分娩。',
    tags: ['妇产科', '产检陪同', '温柔体贴'],
    rating: 98.0,
    orderCount: 623,
    status: 'active',
    hospitals: ['北京妇产医院', '北大第一医院'],
  },
  {
    name: '郑涛',
    gender: 'male',
    phone: '13800138010',
    level: 'intermediate',
    experience: '4年',
    introduction: '骨科陪诊专长，熟悉骨科检查和手术流程。',
    tags: ['骨科专长', '手术陪护', '康复指导'],
    rating: 96.8,
    orderCount: 412,
    status: 'inactive', // 暂停服务
    hospitals: ['积水潭医院', '北京大学第三医院'],
  },
]
```

---

## 六、开发计划

### 6.1 执行时间表

| 阶段 | 任务 | 预计耗时 |
|------|------|----------|
| **Day 1 上午** | 后端 CRUD API + DTO | 2h |
| **Day 1 下午** | 关联医院 API + 种子数据 | 2h |
| **Day 2 上午** | 前端 API + Hooks | 1h |
| **Day 2 下午** | 创建/编辑对话框 | 3h |
| **Day 3** | 删除功能 + 状态管理 + 测试 | 3h |

### 6.2 开发顺序

```
1. 后端 API (Day 1)
   ├── admin-escorts.service.ts (扩展 CRUD)
   ├── admin-escorts.controller.ts (新增端点)
   ├── dto/escort.dto.ts (创建 DTO)
   └── seed.ts (添加种子数据)
   
2. 前端对接 (Day 2)
   ├── src/lib/api.ts (扩展 escortApi)
   ├── src/hooks/use-api.ts (添加 mutations)
   ├── escorts/data/schema.ts (表单验证)
   └── escorts/components/escort-form-dialog.tsx
   
3. 功能完善 (Day 3)
   ├── 删除确认对话框
   ├── 批量操作
   └── 联调测试
```

---

## 七、UI 设计参考

### 7.1 列表页改进

```
┌─────────────────────────────────────────────────────────────────────┐
│ 陪诊员列表                                    [筛选] [+新增陪诊员]   │
├─────────────────────────────────────────────────────────────────────┤
│ 状态: [全部 ▼]  等级: [全部 ▼]  搜索: [_____________] 🔍           │
├─────────────────────────────────────────────────────────────────────┤
│ □ │ 头像  │ 姓名     │ 等级   │ 手机        │ 服务数 │ 评分  │ 状态  │
│───┼───────┼──────────┼────────┼─────────────┼────────┼───────┼───────│
│ □ │ [👤]  │ 张晓红   │ 资深   │ 138****8001 │ 1258   │ 99.2% │ ● 活跃│
│ □ │ [👤]  │ 李明华   │ 资深   │ 138****8002 │ 876    │ 98.5% │ ● 活跃│
│ □ │ [👤]  │ 王芳     │ 中级   │ 138****8003 │ 542    │ 97.8% │ ● 活跃│
│ □ │ [👤]  │ 郑涛     │ 中级   │ 138****8010 │ 412    │ 96.8% │ ○ 暂停│
└─────────────────────────────────────────────────────────────────────┘
```

### 7.2 新增/编辑对话框

```
┌─────────────────────────────────────────────────────────────────────┐
│ 新增陪诊员                                              [×]         │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ┌───────────┐  姓名: [张晓红______]    性别: ● 女 ○ 男            │
│  │  上传头像  │  手机: [13800138001__]  身份证: [110101********1234]│
│  │   点击上传 │  等级: [资深 ▼_______]  从业年限: [8年 ▼]          │
│  └───────────┘                                                      │
│                                                                     │
│  个人简介:                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐│
│  │从事医疗陪诊服务8年，熟悉北京各大三甲医院就诊流程...               ││
│  └─────────────────────────────────────────────────────────────────┘│
│                                                                     │
│  服务标签:                                                          │
│  [耐心细致 ×] [经验丰富 ×] [三甲医院专家 ×]                         │
│  [输入新标签______] [添加]                                          │
│                                                                     │
│  关联医院:  [+添加医院]                                             │
│  ┌──────────────────────────────────────────────────────────┐      │
│  │ ☑ 北京协和医院           熟悉科室: [内科][外科][+]       │      │
│  │ ☑ 北京大学第一医院       熟悉科室: [心内科][+]           │      │
│  └──────────────────────────────────────────────────────────┘      │
│                                                                     │
│  资质证书:  [+上传证书]                                             │
│  ┌──────────────────────────────────────────────────────────┐      │
│  │ 📄 护士资格证 (有效期: 2025-12-31)                   [×] │      │
│  │ 📄 健康证 (有效期: 2024-06-30)                       [×] │      │
│  └──────────────────────────────────────────────────────────┘      │
│                                                                     │
├─────────────────────────────────────────────────────────────────────┤
│                                           [取消]  [保存]            │
└─────────────────────────────────────────────────────────────────────┘
```

---

## 八、风险与注意事项

### 8.1 数据完整性

- **删除保护**: 有进行中订单的陪诊员不能删除
- **手机号唯一**: 创建/编辑时需检查手机号是否已存在
- **状态流转**: `pending` → `active` → `inactive`/`suspended`

### 8.2 JSON 字段处理

- 后端存储时 `JSON.stringify()`
- 前端显示时解析 `JSON.parse()`
- 注意空值处理 (`null` vs `[]` vs `""`)

### 8.3 性能考虑

- 医院列表可能很长，考虑分页或虚拟滚动
- 陪诊员列表默认按 `createdAt DESC` 排序
- 前端表格已支持客户端排序/筛选

---

## 九、验收标准

- [ ] 可创建陪诊员并关联医院
- [ ] 可编辑陪诊员基本信息和关联医院
- [ ] 可删除无订单的陪诊员
- [ ] 可快速切换陪诊员状态
- [ ] 列表支持按状态/等级筛选
- [ ] 列表支持按姓名/手机搜索
- [ ] 种子数据成功导入 10 条记录

---

**文档完成，等待审计** 📝

