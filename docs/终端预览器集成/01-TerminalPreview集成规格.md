...
### 预览器 vs 真实终端的 viewerRole 边界

| 场景 | viewerRole 来源 | 是否允许手动切换 |
|------|----------------|-----------------|
| **真实终端** | 由 `escortToken` 的 validate 结果推导 | ❌ 禁止手动写入 |
| **预览器 DebugPanel** | 可通过 UI 强制模拟 | ✅ 仅用于预览调试 |

> 🚨 **强制约束**：DebugPanel 的"ViewerRole 切换"**只用于预览模拟**。真实终端的 `viewerRole` 只能由 `escortToken` 的 validate 结果推导，**不允许手动写入持久化状态**。禁止将 DebugPanel 的切换逻辑搬到真实终端，否则会导致越权风险。
...
> 🚨 **这是强制规范，避免"mock token 调真实后端"的安全风险**

| 场景 | mock token 用途 | 是否允许调真实后端 |
|------|----------------|-------------------|
| **预览器数据展示** | 用于 previewApi 的 mock 数据通道 | ❌ 禁止 |
| **预览器 UI 切换** | 用于 viewerRole 模拟和 UI 渲染 | ❌ 禁止 |
| **联调测试** | 必须使用真实 token | ✅ 允许 |

#### 强制规则

1. **mock token 只能用于预览器内部逻辑**，不允许拿 mock token 调真实后端 API
2. **预览器的 API 调用必须区分**：
   - 有真实 token → 调真实后端
   - 只有 mock token → 使用静态 mock 数据或 mock server
3. **预览器不应产生真实副作用**：mock 模式下禁止调用有写操作的 API

#### 实现建议

```typescript
// previewApi 的 mock 模式检测
function isMockToken(token: string | undefined): boolean {
  return token?.startsWith('mock-') ?? false
}

// 请求时判断
async function previewRequest<T>(endpoint: string, token?: string): Promise<T> {
  if (isMockToken(token)) {
    // 返回静态 mock 数据
    return getMockData<T>(endpoint)
  }
  // 调用真实 API
  return realRequest<T>(endpoint, token)
}
```
