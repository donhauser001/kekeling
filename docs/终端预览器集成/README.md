...
| 阶段 | 周期 | 目标 | 优先级 | 依赖 |
|------|------|------|--------|------|
| **阶段一-A** | 第 1 周 | 双会话 + 请求分流 + DebugPanel（P0 先闭环） | P0 | 无 |
| **阶段一-B** | 第 1-2 周 | 营销中心页面接入（可与 A 并行，但必须使用 request 封装） | P0 | 阶段一-A 的 request 封装 |
| 阶段二 | 第 3-4 周 | 陪诊员系统集成 + 工作台 | P1 | 阶段一完成 |
| 阶段三 | 第 5 周 | 优化和测试 | P2 | 阶段二完成 |

> ⚠️ **关键依赖**：阶段一-B 的页面接入**必须使用 `request` 封装**，禁止直接 fetch/axios，否则会出现"先接页面但还没分流封装"的返工风险。
