...
| 环境 | userToken 存储 | escortToken 存储 |
|------|---------------|-----------------|
| **小程序** | `wx.setStorageSync('userToken', token)` | `wx.setStorageSync('escortToken', token)` |
| **H5 终端** | `localStorage.setItem('userToken', token)` | `localStorage.setItem('escortToken', token)` |
| **管理后台预览器** | 通过 Props 注入 mock token，**不落地存储** | 通过 Props 注入 mock token，**不落地存储** |

> ⚠️ **强制规范**：全端统一 key 名称：`userToken` / `escortToken`，禁止自定义命名。
