 ...
 * 小程序图标资源
 *
 * 使用 Base64 编码的 SVG 图标，避免网络请求
 * 图标来源：基于 Lucide 图标设计风格
 */

// SVG 模板生成器
const createSvg = (path: string, strokeWidth = 2) => {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="${strokeWidth}" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`
  return `data:image/svg+xml;base64,${btoa(svg)}`
}

// 带颜色的 SVG 生成器
export const createColoredSvg = (path: string, color: string, strokeWidth = 2) => {
